...
python cli.py --bootstrap --force    # 首次建立数据：回溯已结束活动的公告，补录剿灭/保全等长期任务
```
...
- 列表为空时，**第一个发 `/方舟日程` 的会话会被自动填进去**，不用手填会话 ID；
- 之后在设置页里增删即可——**想停掉某个群，把那一项删掉**；
- 推送分两条：先文字，再图片。**文字只在关键节点上出现**——活动「剩 3 天／明天结束／今天开启」
  这类节点命中才发（节点可在配置的 `notify` 一节点自己调），平时就只发图、一个字都不发。
...
| `push.targets` | list | （空） | 推送目标会话；为空时第一个发 `/方舟日程` 的会话会自动加进来，之后在这里增删 |
