...
注意：本文件用**绝对导入**（`from src...`）：它是以脚本方式运行的，
`__main__` 没有父包，相对导入会失败。插件入口 main.py 则相反。
"""
...
def main(force: bool = False, bootstrap: bool = False, 现在时间: datetime | None = None) -> None:
    ...
    # 第 3~6 步交给流水线：过滤 → 主题 → 渲染 → 节点提醒（顺带产出当天日差）
    结果 = render_once(
        现在时间=现在时间,
        强制刷新=False,          # 数据新鲜度已在上两步判定
        控制台打印提醒=True,     # CLI 把提醒打到 stdout
    )
    # 日差仍然产出（不进推送，但 CLI 顺手打一行便于对账）
    if 结果.变化:
        print("\n" + 结果.变化)
    return 结果
