...
| 没做的 | 为什么 |
|---|---|
| **不用配置填目标会话** | 谁发过 `/甘特图` 就记住谁（`event.unified_msg_origin`）。用户不必知道 umo 是什么，也不必"抄一段 ID 再填进配置"——参考物在 schema 里让用户发一条它自己都没实现的 `/sid` 指令，是反面教材 |
...
| **不加 `__del__`** | `star_manager.py` 是 `if __del__ … elif terminate`——加了 `__del__` 会让 `terminate()` 永不执行，热重载后变成双实例双推送（§16） |

## 必须做对的三件事

1. **幂等**：热重载/重启后可能出现两个实例，靠磁盘上的"已推记账"（`{umo: 日期}`）兜住，
   **只在投递成功后记账**。状态文件与渲染缓存同一个数据目录，`fsync` + `os.replace` 原子写。
...
直接增删——"想停掉某个群的推送就把它删掉"。这里只留"今天推过谁"的记账。
    ...
    ```json
    {
      "会话": ["aiocqhttp:GroupMessage:200000"],     // 谁用过 /甘特图（最近在后）
...
class 推送服务:
    ...
    @staticmethod
    def _文案(结果) -> str:
        """推送附带的文字：**先日差，再真有事时的提醒**；都没有就一句都不发（只发图）。

        每天固定来一句"当前所有行动均在正常排期内"是纯噪音（§17.4），
        所以 `渲染结果.有警告` 为假时直接丢掉那句回落文案。
        """
        段 = [getattr(结果, "变化", ""),
              结果.警告 if getattr(结果, "有警告", False) else ""]
        return "\n".join(s for s in 段 if s)
