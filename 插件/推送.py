...
| 没做的 | 为什么 |
|---|---|
| **不自己记住会话**（阶段六改成配置项） | 起初的写法是"谁发过 `/甘特图` 就记住谁"（`event.unified_msg_origin`）。但用户要**能在设置页里删掉某个群的推送**，于是目标会话搬进配置项 `push.targets`：列表为空时把第一个用指令的会话自动填进去，之后由用户在设置页增删（§8） |
...
| **不加 `__del__`** | `star_manager.py` 是 `if __del__ … elif terminate`——加了 `__del__` 会让 `terminate()` 永不执行，热重载后变成双实例双推送（§9） |
