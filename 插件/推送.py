...
无提醒说明 = "今日无节点提醒，按「只在有节点提醒时推送」静默（连图都不发）"
"""`notify.require_reminder` 生效、当天一个提醒都没命中时的结果说明。

它同时进 `推送状态.json` 的「上次.说明」与 `/甘特图状态` —— 用户要能一眼看出
"今天为什么群里一条消息都没有"，而不是以为机器人挂了。
"""
...
class 推送服务:
    ...
    def 一句话(self, 运行配置: 运行配置) -> str:
        ...
        if 运行配置.只在有提醒时推送 and not 运行配置.通知开关:
            # 组合陷阱：没有提醒 = 永远没有推送。不猜用户意图，但必须让人看得见
            return (f"推送：每天 {运行配置.推送时刻}，但「节点提醒总开关」关着 → "
                    "「只在有节点提醒时推送」永远不成立，等于不推")
    ...
    async def 推一次(self, *, 运行配置: 运行配置) -> str:
        ...
        async with self._任务锁:
            ...
            # 今天没有节点提醒 → 整次静默（连图都不发）。判据就是渲染结果里的 有提醒。
            # ⚠️ 判定在渲染**之后**：图已经画好了，这里省的是投递、不是渲染
    ...
    @staticmethod
    def _文案(结果, 运行配置: 运行配置 | None = None) -> str:
        """推送附带的文字：**只有节点提醒**，没有就一句都不发（只发图）。

        - 不再带日差（"今天和昨天比变了什么"）——2026-09-15 定：只在关键节点说话；
        - `渲染结果.有提醒` 为假时直接丢掉那句回落文案，否则每天固定发一句废话（docs「两次自我推翻」）；
        - 2026-09-23 起可自定义：`push.text_template`（默认 `{提醒}`），占位符 `{提醒}` / `{日期}` / `{条目数}`。
          用 `str.replace` 而**不是** `str.format`——用户模板里写别的 `{}`（颜文字之类）不该炸；
          模板留空 = 不附文字（等价于关掉文字）。
        """
        if not getattr(结果, "有提醒", False):
            return ""
        模板 = 运行配置.推送文案模板 if 运行配置 is not None else "{提醒}"
        替换 = {
            "{提醒}": 结果.提醒,
            "{日期}": datetime.now().strftime("%m-%d"),
            "{条目数}": str(getattr(结果, "条目数", "")),
        }
        文本 = 模板
        for 占位符, 值 in 替换.items():
            文本 = 文本.replace(占位符, 值)
        return 文本
