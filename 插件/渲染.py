...
3. **按需渲染，不留缓存**：请求来就画，直接原子落到 `settings.output_path`（`Gantt.jpg`）。
   每天真正有价值的信息是"和昨天比变了什么"，那由 `src/活动快照.py` 回答（`渲染结果.变化`），
   而不是靠复用昨天的图。
...
class 渲染服务:
    ...
    def 清理旧缓存(self) -> None:
        ...
        只删这一个目录名，不动同级的 `历史/`（数据快照）与 `图片缓存/`（头像）。
        """
        目录 = self.数据目录 / "渲染缓存"
        if 目录.is_dir():
            try:
    ...
    def 内核设置(self, 运行配置: 运行配置) -> Settings:
        ...
        - 数据（CSV/预告/图标缓存/警告/产物/历史快照）全在 `data/plugin_data/<插件名>/`
          ——用户数据与代码分离，升级插件不覆盖；
    ...
    def 上次快照日期(self) -> str:
        """最近一份数据快照的日期（给 `/甘特图状态` 用）"""
        from ..src.活动快照 import 上次快照日期

        return 上次快照日期(self.数据目录 / "历史")
