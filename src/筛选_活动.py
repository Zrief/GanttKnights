...
改造自 pandas 版（见 docs/插件化.md 阶段二·五）。行为必须与旧实现等价，
三个容易改错的点已就地标注：
...
def _反推子类型(名称: str) -> str:
    """从名字前缀反推卡池子类型 —— **只为旧数据**。

    ⏳ 时效（2026-09-24 加）：`子类型` 是这一天才进的 CSV 列，之前的行没有它。
    这份反推只是让旧数据也能立刻按池子分组、不必等重抓；**等 CSV 全量重抓过一次**
    （`/甘特图初始化`，或旧条目自然过期后）就可以连同这个函数一起删掉——
    那时每一行都带着 `子类型`，这条路径永远走不到。

    只认「中坚池」那类前缀，其余一律返回空串（活动/福利/长期本来就不细分）。
    """
    for 前缀, 子类型 in (("【寻访】", "限时"), ("【限时】", "限时"),
                        ("【中坚池】", "中坚"), ("【中坚】", "中坚"),
                        ("【标准池】", "标准"), ("【标准】", "标准")):
        if 名称.startswith(前缀):
            return "甄选" if "甄选" in 名称 else 子类型
    return ""
        ...
        with open(路径, newline="", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                ...
                记录.append(活动(
                    ...
                    结束=结束,
                    类型=类型,
                    子类型=(row.get("子类型") or "").strip() or _反推子类型(名称),
