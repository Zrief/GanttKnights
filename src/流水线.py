...
两边都用 `今天写过()` 判断新鲜度：CLI 每次跑就是一次，插件则由 `渲染服务`
对活动数据（CSV）与首页预告（`新增预告.json`）各判一次。

## 关于重依赖的延迟导入（重要）

本模块**故意不在顶层 import 会拖入 matplotlib 的子模块**（绘图_图表 / 绘图_主题 /
绘图_排版 / 字体）。原因：同步 `import matplotlib` 实测会让 AstrBot 事件循环
停顿 0.5s 以上（见 docs/插件化.md「起点」），而插件加载期是同步执行的。

顶层只保留"轻"依赖（config / 获取_prts / 汇总_活动 / 解析_* / 生成_提醒）；
...
无提醒文案 = "博士，罗德岛当前所有行动均在正常排期内，无需提醒。"
"""没有任何节点命中时的回落文案。

单独拿出来是为了让调用方判得出"这句话有没有信息量"——每日推送只在**真有事**时才附文字
（`渲染结果.有提醒`），否则每天固定发一句废话（docs「两次自我推翻」）。"""


@dataclass(frozen=True)
class 渲染结果:
    """render_once() 的产出：产物路径 + 可播报的概要 + 今天的日差"""
    ...
    分区计数: dict[str, int]
    变化: str = ""
    """今天的日差（`较 09-14：🆕 甲、乙｜🧹 清理 3`）；没有变化/首次记录时为空串。

    只在日志、`/甘特图状态`（日期）与 `/甘特图初始化` 的回显里用；**不进每日推送**（2026-09-15 定）。"""

    有提醒: bool = False
    """`提醒` 是否言之有物（不是"无需提醒"那句回落）——推送据此决定要不要发文字。"""
...
def 更新增预告(现在时间: datetime, 现在字符串: str) -> dict:
    ...
    if not any(新增.values()):
        # 覆盖为带时间戳的空预告：底栏三区看到"这块为空"就不再画（不会长期挂着早已结束的上新）
...
def 筛选底栏分区(新增内容: dict, 底栏分区: tuple[str, ...] | None) -> dict:
    """只保留要展示的底栏分区：未选中的板块置空 → 底栏不画它。

    参数用 绘图_排版.区名表 里的名字（"凭证兑换" / "新增时装" / "新增模组"）；
    None = 三个都要，原样返回（默认路径行为不变）。
    置空（而不是删键）是为了让 `建分区()` 的"空板块"处理照旧生效。
    """
    if 底栏分区 is None:
        return 新增内容
...
def render_once(
    ...
    """数据 → 图片 + 节点提醒。不碰 argparse，也不判断"今天要不要爬"。
    ...
    参数:
        ...
        标题          : 图片主标题
        结束偏移们    : 距结束几天时提醒（空元组 = 这类不提醒）
        开始偏移们    : 距开始几天时提醒（空元组 = 这类不提醒）
        底栏分区      : 要展示的底栏分区（"凭证兑换"/"新增时装"/"新增模组"）；
                        None = 三个都展示（默认）。未选中的区不画。
        控制台打印提醒: 是否把提醒同时打到 stdout（CLI 用；插件应关掉）
    """
    ...
    # 节点提醒（只在配置的天数节点上说话；同一天里渲染多少次都是同一份文案）
    提醒 = ""
    try:
        提醒 = 生成提醒(记录, 现在时间=现在时间,
                        结束偏移们=结束偏移们, 开始偏移们=开始偏移们)
        if not 提醒:
            提醒 = 无提醒文案
        if 控制台打印提醒:
            print("\n" + "=" * 54)
