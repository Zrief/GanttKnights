...
docs/插件化路线.md §10）：`main.py` 导入期 `import matplotlib`，缺依赖时宿主才会替我们跑
`requirements.txt` 并重试导入。删掉它不会有任何报错，只会让"缺依赖"重新变成用户要自己处理的故障，
所以用测试钉住——顺便钉住 `MPLCONFIGDIR` 必须**先于**它被设置（§10 的顺序约束）。
...
def test_导入期_import_matplotlib():
    """缺依赖时唯一能让宿主替我们装依赖的信号，就是"插件导入失败"（§10）。"""
    assert "matplotlib" in _顶层导入(), (
        "main.py 导入期的 import matplotlib 没了：缺依赖时插件会照常加载，"
        "宿主收不到任何信号，用户只能到第一次出图才看到失败（docs/插件化路线.md §10）"
    )


def test_mplconfig_先于_matplotlib_导入():
    """`MPLCONFIGDIR` 只在 matplotlib 首次导入时被读取（§10）：顺序反了就等于没设。"""
    文本 = 源码.splitlines()
...
def test_requirements_写上了宿主不带的两个():
    """AstrBot 自带 httpx / pillow / apscheduler，**不带 numpy / matplotlib**（§10）。
