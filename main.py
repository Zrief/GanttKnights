...
* **导入期只碰 matplotlib 一个重依赖**：它在顶层就被 import（缺依赖时宿主据此自动装依赖，「装得上、上得了架」），
  而 pyplot 与绘图模块仍留在渲染线程里（「起点」），`initialize()` 立即返回；
* **一切阻塞都在 `asyncio.to_thread` 里**：渲染是秒级 CPU 占用，取数是同步 httpx（「接上宿主」）；
...
# ==================== 依赖：按生态的约定俗成来（2026-09-15 定，见 docs/插件化.md「装得上、上得了架」）====================
#
...
#
# ⚠️ `MPLCONFIGDIR` 必须在 matplotlib **首次导入之前**设好（「踩坑清单」）：容器/只读环境里默认配置目录
# 不可写，会报错或反复重建字体缓存。
#
...
class GanttKnightsPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig | None = None):
        ...
        # 不让一个加载期 TypeError 把插件整个毙掉（「踩坑清单」）。
        ...
        # 插件名优先用 AstrBot 注入的 self.name，取不到时回落常量（「宿主硬事实」）。
    ...
    async def terminate(self) -> None:
        ...
        跨重启自然复用；渲染产物不在状态里，下次出图重画一遍即可（「两次自我推翻」）。

        ⚠️ 不要给这个类加 `__del__`：`star_manager.py` 是 `if __del__ … elif terminate`，
        加了之后 `terminate()` 永远不会执行（「宿主硬事实」）。
        """
        await self.推送.停()
        logger.info("明日方舟甘特图已卸载。")
    ...
    async def _发送(self, unified_msg_origin: str, 文本: str, 图片路径: str) -> bool:
        ...
        为什么不合成一条 MessageChain：不少平台会把"图 + 文"合成一条带配文的图，
        图片被压小、文字也被折叠；分开发更像"先说一句、再给图"，读起来清楚
        （用户 2026-09-15 定）。

        - 只用 `unified_msg_origin`（裸 session_id 会在宿主的 `split(":", 2)` 处炸，「宿主硬事实」）；
        ...
        - 不预检平台能力：`support_proactive_message` 是 4.28 新增且默认 True（信不过），
          发一次并把结果记下来比读字段可靠（「每日推送」）。
        """
        if 文本:
            结果 = await self.context.send_message(
                unified_msg_origin, MessageChain([组件.Plain(text=文本)])
            )
            if 结果 is False:
                logger.warning("文字未投递（未找到平台实例）：%s", unified_msg_origin)
                return False
        结果 = await self.context.send_message(
            unified_msg_origin, MessageChain([组件.Image.fromFileSystem(图片路径)])
        )
        if 结果 is False:
            logger.warning("图片未投递（未找到平台实例）：%s", unified_msg_origin)
            return False
        return True
    ...
    def _状态文本(self, event: AstrMessageEvent) -> str:
        ...
        if 数据.exists():
            ...
            新鲜 = "今天" if 时刻.date() == datetime.now().date() else "不是今天"
            数据行 = f"{时刻:%m-%d %H:%M}（{新鲜}）"
        else:
            数据行 = "还没有（首次出图时抓取）"
        ...
        上次行 = ""
        if 上次:
            时刻 = str(上次.get("时刻", "?"))[5:16].replace("T", " ")
            成功, 失败 = 上次.get("成功", 0), 上次.get("失败", 0)
            说明 = 上次.get("说明", "")
            if not 成功 and not 失败 and 说明:
                # 静默日（当天没有节点提醒）：报"成功 0 / 失败 0"只会让人以为出了问题
                上次行 = f"\n上次：{时刻} {说明}"
            else:
                上次行 = f"\n上次：{时刻} 成功 {成功} / 失败 {失败}"
                if 说明:
